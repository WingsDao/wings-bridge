...
Wings bridge contract works like communication contract. It allows to notify Wings about new contributions during crowdsale and about its state.
...
- Call method `closeBridge` when crowdsale completes and implement movement of rewards to bridge contract.
- Deploy your token/crowdsale contracts, deploy bridge contract, call method `changeBridge` in your crowdsale contract and pass there address of deployed bridge contract.
- Create project at Wings platform as custom contract and provide address of **bridge** contract.
- Call `transferManager` method in bridge contract and pass there DAO contract address generated by Wings as an argument.
- Call `start` in your bridge contract **before** crowdsale starts.
...
To accomplish this, you need to call few methods on DAO contract, indeed `createCustomCrowdsale` method and  `start` method in your crowdsale controller contract, that will be generated by `createCustomCrowdsale` call.
